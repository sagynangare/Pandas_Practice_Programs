...
for d in range(1,30):
    ...
    for df in dfs:
        for j in range(1,289):
            c.execute("INSERT INTO stuffToPlot (TIMESLOT , DELHI , BRPL , BYPL , NDPL, NDMC , MES) VALUES (?, ?, ?, ?, ?, ?, ?)", (str(df[0:j]) , str(df[1:j]) , str(df[2:j]) , str(df[3:j]) , str(df[4:j]), str(df[5:j]) , str(df[6:j])))
            c.commit()
